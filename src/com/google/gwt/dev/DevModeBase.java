...
public abstract class DevModeBase implements DoneCallback {
  ...
  public class UiBrowserWidgetHostImpl implements BrowserWidgetHost {
    ...
    public ModuleSpaceHost createModuleSpaceHost(ModuleHandle module, String moduleName)
      ...
      try {
        ...
        Method getCompilationStateMethod  = null;
        Object[] args = null;
        try {
          getCompilationStateMethod = moduleDef.getClass().getMethod("getCompilationState", TreeLogger.class, boolean.class);
          args = new Object[]{logger, !options.isStrict()};
        } catch (NoSuchMethodException e) {
          try {
            getCompilationStateMethod = moduleDef.getClass().getMethod("getCompilationState", TreeLogger.class, compilerContext.getClass());
            args = new Object[]{logger, compilerContext};
          } catch (NoSuchMethodException e1) {
            throw new RuntimeException(e1);
          }
        }
        try {
          compilationState = (CompilationState) getCompilationStateMethod.invoke(moduleDef, args);
        } catch (Exception e) {
  ...
  protected static class ArgHandlerCodeServerPort extends ArgHandlerString {
    ...
    public String getPurpose() {
      return "Specifies the TCP port for the code server (defaults to " + DEFAULT_PORT
          + " for classic-devMode or 9876 for super-devMode)";
    }
  ...
  public DevModeBase() {
    ...
    try {
      ...
      compilerContextBuilder = builderClazz.newInstance();
      compilerContext = buildMethod.invoke(compilerContextBuilder);
      System.out.println("Running GPE launcher for GWT-2.6.x version.");
    } catch (Exception e) {
  ...
  protected ModuleDef loadModule(TreeLogger logger, String moduleName, boolean refresh)
    ...
    Method loadFromClassPathMethod = null;
    try {
      loadFromClassPathMethod = moduleDefLoaderClazz.getMethod("loadFromClassPath", TreeLogger.class, String.class, boolean.class);
      moduleDef = (ModuleDef) loadFromClassPathMethod.invoke(null, new Object[] {logger, moduleName, refresh});
    } catch (Exception e) {
      try {
        loadFromClassPathMethod = moduleDefLoaderClazz.getMethod("loadFromClassPath", TreeLogger.class, compilerContext.getClass(), String.class, boolean.class, boolean.class);
        moduleDef = (ModuleDef) loadFromClassPathMethod.invoke(null, new Object[] {logger, compilerContext, moduleName, refresh, true});
        Class<?> builderClazz = Class.forName("com.google.gwt.dev.CompilerContext$Builder");
        Method buildMethod = builderClazz.getMethod("build");
        Method moduleMethod = builderClazz.getMethod("module", moduleDef.getClass());
        moduleMethod.invoke(compilerContextBuilder, new Object[] {moduleDef});
        compilerContext = buildMethod.invoke(compilerContextBuilder);
      } catch (Exception e2) {
        e2.printStackTrace();
      }
